...
void count_src_update(u_char *packet) {
  ...
  std::unordered_map<unsigned long, long>::iterator it = state.state_map.find(srcIP);
  if (it == state.state_map.end()) { 
    state.state_map.insert(std::pair<unsigned long, long>(srcIP, 1));
  } else {
    it->second += 1;
  }
}
